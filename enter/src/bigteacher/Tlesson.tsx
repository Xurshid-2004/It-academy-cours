...
import { useAddLessonMutation } from "./teacherApi";
...
const Tlesson = (props: any) => {
  ...
  const saveModule = async (item: any) => {
    try {
      const dataToSend = {
        name: item.name, description: item.age, day: item.day,
        type: item.select, url: item.url, video: item.vidyo,
        teacherId: localStorage.getItem("id")
      };
      await addLesson(dataToSend).unwrap();
      ...
      <div className={`relative z-10 w-full flex flex-col items-center py-6 gap-6 h-screen ${modules.length > 1 ? 'overflow-y-auto' : 'overflow-hidden'} custom-scrollbar`}>
        ...
        {modules.map((item) => (
          <div key={item.id} className="relative mb-6">
            ...
            <div className="bg-grenn w-[900px] min-h-[350px] border border-white/10 rounded-2xl flex justify-center items-start gap-6 p-6 backdrop-blur-xl shadow-2xl">
              
              {/* 1-CARD: MAIN (Inputlar mayda) */}
              <div className="w-[300px] bg-black/50 p-1 rounded flex flex-col gap-3">
                ...
                >
                  <option value="" disabled>choos lesson</option>
                  <option value="frontend">Frontend</option>
                  <option value="backend">Backend</option>
                  <option value="devops">DevOps</option>
                  <option value="devops">Security</option>
                  <option value="devops">Dev Ops</option>
                  <option value="devops">Mobile devoloper</option>
                  <option value="devops">database</option>
                </select>
              ...
              {item.listOpen && (
                <div className="bg-black/50 flex flex-col gap-12 p-3 rounded-xl w-[280px] ">
                  ...
                  />
                  <input className="w-full p-2 text-xs rounded-lg bg-white/5 border border-white/10 text-white" placeholder="Resurs linki" />
                </div>
              )}
